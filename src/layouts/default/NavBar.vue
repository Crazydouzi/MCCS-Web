<template>
  <v-navigation-drawer app :model-value="store.isDrawer" color="red">
    <v-list density="compact">
      ...
      <v-list-item class="mt-2" prepend-icon="mdi-cube-outline" title="实例管理" value="Server"
        @click="router.push({ name: 'Server' })"></v-list-item>
      <v-list-item class="mt-2" prepend-icon="mdi-console" title="CMD" value="Command"
        @click="router.push({ name: 'Command' })"></v-list-item>
      <v-list-item class="mt-2" prepend-icon="mdi-cog-outline" title="系统管理" value="System"
        @click="router.push({ name: 'System' })"></v-list-item>
      <v-list-item class="mt-2" prepend-icon="mdi-logout-variant" title="登出" @click="logout()"></v-list-item>
    </v-list>
  </v-navigation-drawer>
</template>
